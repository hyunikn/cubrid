...
static int au_grant_procedure (DB_OBJECT_TYPE obj_type, MOP user, MOP obj_mop, DB_AUTH type, bool grant_option);

static int au_revoke_class (MOP user, MOP class_mop, DB_AUTH type, MOP drop_user);
static int au_revoke_procedure (DB_OBJECT_TYPE obj_type, MOP user, MOP obj_mop, DB_AUTH type, MOP drop_user);
...
au_grant (DB_OBJECT_TYPE obj_type, MOP user, MOP class_mop, DB_AUTH type, bool grant_option)
{
  ...
  switch (obj_type)
    ...
    case DB_OBJECT_PACKAGE:
      error = au_grant_procedure (obj_type, user, class_mop, type, grant_option);
      break;
    default:
      error = ER_FAILED;
      assert (false);
...
static int
au_grant_procedure (DB_OBJECT_TYPE obj_type, MOP user, MOP obj_mop, DB_AUTH type, bool grant_option)
{
  int error = NO_ERROR;
  DB_VALUE value;
  MOP auth;
...
au_revoke (DB_OBJECT_TYPE obj_type, MOP user, MOP obj_mop, DB_AUTH type, MOP drop_user)
{
  ...
  switch (obj_type)
    ...
    case DB_OBJECT_PROCEDURE:
    case DB_OBJECT_PACKAGE:
      error = au_revoke_procedure (obj_type, user, obj_mop, type, drop_user);
      break;
    default:
      error = ER_FAILED;
      assert (false);
...
static int
au_revoke_procedure (DB_OBJECT_TYPE obj_type, MOP user, MOP obj_mop, DB_AUTH type, MOP drop_user)
{
  int error = NO_ERROR;
  DB_SET *grants = NULL;
  MOP auth;
      ...
      else if ((error = obj_inst_lock (auth, 1)) == NO_ERROR && (error = get_grants (auth, &grants, 1)) == NO_ERROR)
	{
	  ...
	  if (gindex == -1)
	      ...
	      else if ((error = collect_class_grants (obj_mop, type, auth, gindex, &grant_list)) == NO_ERROR)
		{
		  ...
		  /* propagate the revoke to the affected classes */
		  if ((error = propagate_revoke (obj_type, grant_list, sp_owner, (DB_AUTH) mask)) == NO_ERROR)
		    {
...
au_check_execute_authorization (MOP obj_mop, DB_OBJECT_TYPE obj_type)
{
  ...
  uint32_t *bits = Au_cache.get_procedure_cache_bits (obj_mop);
  if (bits == NULL)
    {
      assert (false);
      return er_errid ();
  ...
  if ((*bits & AU_EXECUTE) != AU_EXECUTE)
	...
	{
	  error = db_get (obj_mop, SP_ATTR_OWNER, &owner);
	  ...
	  if (error == NO_ERROR)
	    {
	      bits = Au_cache.get_procedure_cache_bits (obj_mop);
	      if (bits == NULL)
		{
		  return er_errid ();
		}
