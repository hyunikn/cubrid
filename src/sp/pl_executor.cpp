...
exit:
  ...
  executor::callback_check_execute_auth (cubthread::entry &thread_ref, packing_unpacker &unpacker)
  {
    ...
    // the caller's generated code sends the callee routine's canonical unique_name
    std::string unique_name;
    unpacker.unpack_all (unique_name);

    // EXECUTE authorization lives client-side (Au_user is already the definer during execution via
    // METHOD_CALLBACK_CHANGE_RIGHTS), so forward the check to the CAS and relay the verdict back to
    // the PL server, mirroring the static-SQL prepare/execute path.
    auto relay_result = [&] (const cubmem::block & b)
    {
      return m_stack->send_data_to_java (b);
    };
